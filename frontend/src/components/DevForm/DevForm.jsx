...
function DevForm({ onSubmit, dev, usernameDisabled = false, cancelButtonAction }) {
  ...
  useEffect(() => {
    if (dev) {
      ...
      setTechs(dev.techs ? dev.techs.join(', ') : '');
      if (dev.location && dev.location.coordinates) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="input-block">
        <button type="button" onClick={myLocation} disabled={loading}>
          {loading ? 'Aguarde...' : 'Minha localização'}
        </button>
      </div>

      <button type="submit" disabled={loading}>
        {loading ? 'Aguarde...' : dev ? 'Editar' : 'Salvar'}
      </button>
      {cancelButtonAction && (
        <button
          type="button"
